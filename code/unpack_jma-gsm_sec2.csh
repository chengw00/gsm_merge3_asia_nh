...
foreach ff ( $filelist )
 ...
 if ( $variable == "H" ) then
  set VAR = height
  set GNAME = "HGT_38_ISBL"
 else if ( $variable == "R" ) then
  set VAR = rh
  if ( $LEV == "sfc" ) then
   set GNAME = "R_H_38_SFC"
  else
   set GNAME = "R_H_38_ISBL"
  endif
 else if ( $variable == "T" ) then
  set VAR = tmp
  if ( $LEV == "sfc" ) then
   set GNAME = "TMP_38_SFC"
  else
   set GNAME = "TMP_38_ISBL"
  endif
 else if ( $variable == "U" ) then
  set VAR = u
  if ( $LEV == "sfc" ) then
   set GNAME = "U_GRD_38_SFC"
  else
   set GNAME = "U_GRD_38_ISBL"
  endif
 else if ( $variable == "V" ) then
  set VAR = v
  if ( $LEV == "sfc" ) then 
   set GNAME = "V_GRD_38_SFC"
  else
   set GNAME = "V_GRD_38_ISBL"
  endif

 else if ( $variable == "P" ) then
  set VAR = slp
  set GNAME = "PRMSL_38_MSL"
 ...
 if ( ( ! -e $DATA_BIN/$FILE_BIN_OUT ) && ( $VAR != "NONE" ) && ( $LEV != "NONE" ) && ( $FCST != "NONE" ) && ( $CYCLE != "NONE" ) ) then
  echo $VAR $LEV $FCST $GNAME
  ./extract_flip_ncl_sec2.csh $ff $FILE_BIN_OUT $GNAME
